...
import type { Status, TrackerData } from '../types'

const STANDARD = 'BRCGS'

type Row = { id: string; element_code: string; status: Status; notes: string; updated_at: string }

export function useTracker() {
  const { facilityId, loading: facilityLoading } = useFacility()
  const [data, setData] = useState<TrackerData>({})
  const [rowIds, setRowIds] = useState<Record<string, string>>({}) // elementCode → row uuid
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: rows } = await supabase
        .from('checklists')
        .select('id, element_code, status, notes, updated_at')
